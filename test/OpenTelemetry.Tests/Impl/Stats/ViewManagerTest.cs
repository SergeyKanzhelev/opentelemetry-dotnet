            ...
            statsRecorder
                ...
                .Record(
                    tagger
                        ...
                        .Build());
            IViewData viewData = viewManager.GetView(view.Name);
            var tv1 = TagValues.Create(new List<string>() { "v1"), string.Create("v10") });
            var tv2 = TagValues.Create(new List<string>() { "v1"), string.Create("v20") });
            var tv3 = TagValues.Create(new List<string>() { "v2"), string.Create("v10") });
            StatsTestUtil.AssertAggregationMapEquals(
                viewData.AggregationMap,
                new Dictionary<TagValues, IAggregationData>()
                {
